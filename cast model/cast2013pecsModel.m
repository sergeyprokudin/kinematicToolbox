...
%% Rotate the Data Depending on which the participant is walking %%%
[RASI] = structureOutofStructure(MarkerStruct,{'RASI'});
progression=RASI.Data(end,:)-RASI.Data(1,:);
if abs(progression(:,1))>100 & progression(:,1)<0 %#ok<AND2>
     rot2=180;
     RotAboutZ = [cos(rot2*pi/180),-(sin(rot2*pi/180)),0;sin(rot2*pi/180),cos(rot2*pi/180),0;0,0,1];

            for ii=1:length(MarkerStruct)
                Data = [RotAboutZ'*MarkerStruct(ii).Data']'; %#ok<NBRAK>
                MarkerStruct(ii).Data=Data;
            end
...
%% Femur.............................................................
     % Calcualte femur coordinate systems  
        [R_femur] = segmentsystem(RLMAL,RMMAL,RHJC,RKJC,'zxy');
        [L_femur] = segmentsystem(LMMAL,LLMAL,LHJC,LKJC,'zxy');
