function pointer(pointerlength, path2c3dorStructData)
% Pointers for pointData definition
...
elseif nargin == 2
    if ~isstruct(path2c3dorStructData)
        structData = btk_loadc3d(fullfile(path2c3d,10));
    end
else
...
for i=1:length(pntr1)
    ...
    normalmean=-pointerlength(pntr1)*(normalmean/norm(normalmean));
