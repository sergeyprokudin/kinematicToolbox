...
if length(parentMkrs) == 4
     ...
     [staticFrameOrigin, staticFrameOrient] = frameBuilder(staticData,{'midPelvis'}, {parentMkrs{1:2} 'SACR'}, 'v1v3' );

elseif length(parentMkrs) == 3

    staticData =  stationBuilder(staticData, parentMkrs(1:2), {'midPelvis'});
    [staticFrameOrigin, staticFrameOrient] = frameBuilder(staticData,{'midPelvis'}, parentMkrs, 'v1v3' );
