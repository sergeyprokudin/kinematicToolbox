function [MHAFinal] = UWAHelicaltest2011(prox_mkr_trajectories, dist_mkr_trajectories, VideoRate)
...
% prox_mkr_trajectories and dist_mkr_trajectories are strucutres which hold
% the positions of the three markers representing a segment.
...
%% Create xyz 3D column arrays for each prox or distal markers
parentCoords=[prox_mkr_trajectories(1).Data prox_mkr_trajectories(2).Data prox_mkr_trajectories(3).Data];
childCoords=[dist_mkr_trajectories(1).Data dist_mkr_trajectories(2).Data dist_mkr_trajectories(3).Data];

%%   Variables involved in analysis and exclusion of IHA
threshold = 10;
overlap='y';
xAxisTerminal=2;
yAxisOrigin=1;
...
ProxMkr1 = prox_mkr_trajectories(1).Data;
ProxMkr2 = prox_mkr_trajectories(2).Data;
ProxMkr3 = prox_mkr_trajectories(3).Data;

originParent=(ProxMkr1+ProxMkr2+ProxMkr3)/3;

% Calculate unit vectors of proximal origin relative to global
[e1,e2,e3]=segmentorientationV2V1(ProxMkr1-ProxMkr3,ProxMkr2-originParent);
...
%         h1(2) = plot3(childMarker1_PCS(:,1), childMarker1_PCS(:,2),...
%             childMarker1_PCS(:,3), 'ro','MarkerFaceColor', 'r');
%         plot3(childMarker2_PCS(:,1), childMarker2_PCS(:,2),...
%             childMarker2_PCS(:,3), 'ro', 'MarkerFaceColor', 'r')
%         plot3(childMarker3_PCS(:,1), childMarker3_PCS(:,2),...
%             childMarker3_PCS(:,3), 'ro','MarkerFaceColor', 'r')
%         xlabel('X')
%         ylabel('Y')
%         zlabel('Z')
...
%         axis equal
%         grid on
% OPTIONAL: Plot parent and child coords in PCS
