...
%   SAVESTATS (DATA) asks the user to specify a file, and saves 
%   the information in DATA to that file, in ASCII.  The file 
%   is created if it does not exist, or appended to if it is an existing file.
...
dataFile = fopen ([pathname filename], 'at');

% make format string
formatStr = '';

% if it's a cell, could be mix of strings and numbers
if (iscell (data))
...
% if it's not a cell, it's just a matrix
else
