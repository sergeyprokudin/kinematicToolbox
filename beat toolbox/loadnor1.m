function [data, Fs, chanLabel, chanUnits, numChans, numSamples, fullName, ptInfo] = loadnor1 (format, varargin)
% LOADNOR   Load a Noraxon MyoSoft file of analog data in ASCII or binary format
%   [DATA, FS, CHANLABEL, CHANUNITS, NUMCHANS, NUMSAMPLES FULLNAME, PTINFO] = LOADNOR 
...
%   PTINFO is the information about the data
%
%   FULLNAME is the path- and filname of the opened file.
%
%   LOADNOR (TYPE) loads files of type TYPE, where TYPE can be 'ascii' for
%   ASCII exported files and 'myosoft' is MyoSoft's proprietary format.
%
%   LOADNOR (TYPE,FILENAME) loads data from the file FILENAME (note: it is
...
case 'myosoft'
 ...
 %ok another addition to make the filename part work if u have 
 %case 'myosoft'
 %first it checks to see if 2nd argurment is a directory 
        if(exist(theDir) == DIR_EXIST)
            findfile = 1;
 %then it checks to see if 2nd argument was a file in the path
        elseif(exist(startDir) == FILE_EXIST)
            findfile = 0;
            filename = startDir;
            try
            [ptInfo, params, dataStart] = loadmyohdr (filename); 
            catch
            disp('Not a mysoft file!');
            data = [];Fs = [];chanLabel = [];chanUnits = [];numChans = [];
            numSamples = [];fullName = [];ptInfo = [];  
            return
            end            
            [ptInfo, params, dataStart] = loadmyohdr (filename);
 %or was it a file with it's own path           
        elseif(exist(theDir) == FILE_EXIST)
            findfile = 0;
            filename = theDir;
            try
            [ptInfo, params, dataStart] = loadmyohdr (filename); 
...
%otherwise it gives up            
            disp('Specified directory/file does not exist');
            data = [];Fs = [];chanLabel = [];chanUnits = [];numChans = [];
            numSamples = [];fullName = []; ptInfo = [];  
            return
        end
...
%another much needed semicolon below    
    ...
    if (exist (listFile) == FILE_EXIST)
        ...
        fclose (LIST_FILE);
%if there is a ndx file then make this listStr 
        listStr = cellstr (strvcat (fileList, dirs));        
    else
%if there is not a ndx then make this listStr        
        listStr = cellstr(dirs);
    end
    ...
    while (~done)
        ...
        [selection, ok] = listdlg (...
            ...
            'ListString', listStr);
%ok i put a much needed semicolon up here

        % cancel button presed; exit
        if (~ok)
            disp ('no file selected...');
%ok added the initilization of the these variables
%changed the break command to a return so that it doesn't cause the errors
            data = [];Fs = [];chanLabel = [];chanUnits = [];numChans = [];
            numSamples = [];fullName = [];    ptInfo = [];       
            return;
        end
        ...
        if (exist (nextDir) == DIR_EXIST)
            ...
            [data, Fs, chanLabel, chanUnits, numChans, numSamples, fullName,ptInfo] =  ...
                 loadnor1 ('myosoft', nextDir);
        ...
        elseif (strcmp (theButton, 'Cancel'))
            % cancel; just return
%ok addition of this display and initilization of the output variables           
            disp('no file selected');
            data = [];Fs = [];chanLabel = [];chanUnits = [];numChans = [];
            numSamples = [];fullName = [];  ptInfo = [];        
            return;
